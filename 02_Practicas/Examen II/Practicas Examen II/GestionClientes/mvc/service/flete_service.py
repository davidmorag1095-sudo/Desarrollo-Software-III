from mvc.model.flete_model import FleteModel


class FletesService:
    def __init__(self, repository, service_cliente):
        self.repo = repository(FleteModel.from_dict, 'flete.json')
    ...
    def registrar(self, id_flete, id_cliente, ciudad, destino, peso):
        ...
        nuevo = FleteModel(id_flete, id_cliente, ciudad, destino, peso)
    ...
    def buscar(self, id_flete):
        datos_encontrados = self.repo.list_id(id_flete)
        if not datos_encontrados:
